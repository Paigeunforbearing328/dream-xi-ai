...
- 质量门禁层，最后一道防线
- 执行 `pnpm gate` 赛前合并检查
- 自动化测试（单元 + 集成）、TypeScript 严格模式校验
- Biome lint 零警告验证
- 依赖安全扫描与敏感信息检测
- 破坏性变更检测与球队铁律合规检查

## AI 编码助手角色（Antigravity）

Dream XI 通过 **Antigravity AI** 作为 AI 编码助手进行项目建设，与球员团队协同工作。

**Antigravity 在球队中的定位：**

- 担任**技术顾问**——为主教练提供实现建议、架构评审
- 充当**首席文档员**——维护中英双语文档、同步变更记录
- 执行**代码脚手架**——按 SOP 流程创建新模块，符合球衣规范
- 遵守**球队铁律**——不删数据、不杀进程、不改配置、不越端口界

Antigravity 是球队的技术参谋，而非独立球员。主教练（你）的判断和愿景始终优先。
